# Component Updates

출판 전 최신 버전 반영 작업 현황입니다.
각 항목의 상세 내용은 개별 파일을 참고하세요.

## 진행 상태

| 컴포넌트 | 변경 | 상태 | 완료일 |
|---|---|---|---|
| [Ubuntu](ubuntu.md) | 22.04 → 24.04 | ✅ 완료 | 2026-04-04 |
| [Kubernetes](kubernetes.md) | 1.35.0 → 1.36.0 | ✅ 완료 | 2026-05-08 |
| [containerd](containerd.md) | 1.7.x → 2.2.3 | ✅ 완료 | 2026-05-08 |
| [Calico](calico.md) | v3.30.1 → v3.31.2 | ✅ 완료 | 2026-04-04 |
| [MetalLB](metallb.md) | v0.13.10 → v0.15.3 | ✅ 완료 | 2026-04-04 |
| [CSI Driver NFS](csi-driver-nfs.md) | v4.0.0 → v4.12.1 | ✅ 완료 | 2026-04-04 |
| [Helm](helm.md) | v3 → v4.1.3 | ✅ 완료 | 2026-04-04 |
| [kustomize](kustomize.md) | v5.4.1 → v5.8.1 (ch5/5.2.2) | ✅ 완료 | 2026-07-18 |
| [Cilium](cilium.md) | v1.17.4 → v1.17.13 | ✅ 완료 | 2026-04-05 |
| [Docker](docker.md) | 24.0.6 → 29.3.1(테스트) → 26.0.0(최종, ch4~ch6 정책) | ✅ 완료 | 2026-07-17 |
| [Harbor](harbor.md) | v2.10.0 → v2.15.0(테스트) → v2.10.0(최종, 되돌림) | ✅ 완료 (arm64 블로커 해소, 클린 환경 검증, docx 수정 불필요 확인) | 2026-08-08 |
| [metrics-server](metrics-server.md) | v0.6.4 → v0.8.1 (ch3/3.6.1) | ✅ 완료 | 2026-07-13 |
| [nginx rollout 실습](nginx-rollout.md) | 1.23.4/1.24.0/1.25.22 → 1.30.1/1.30.3/1.30.99 (ch3/3.2.4) | ✅ 완료 (yaml + docx 반영 확인) | 2026-08-08 |

## 인프라 도구

| 컴포넌트 | 변경 | 상태 | 완료일 |
|---|---|---|---|
| [VirtualBox](virtualbox.md) | v7.1.10 → v7.2.8 | ✅ 완료 | 2026-05-21 |
| [Vagrant](vagrant.md) | v2.4.7 → v2.4.9 | ✅ 완료 | 2026-05-21 |
| [Tabby](tabby.md) | v1.0.196 → v1.0.234 → v1.0.196 (롤백) | ✅ 완료 (v1.0.196 유지 확정, 업그레이드 재시도 안 함) | 2026-08-08 |

## 기타

| 항목 | 내용 | 상태 |
|---|---|---|
| [블루그린 파이프라인](bluegreen-pipeline.md) | Jenkinsfile 저장소 이름 수정 (ch5/5.4.3) | ✅ 완료 (저장소 + docx 2곳 + 파이프라인 9~15번 통과, 편집자 재실습 정상) |
| [멀티 플랫폼 이미지](multiarch-images.md) | arm64 전수 감사 → kustomize, nginx-status 2건 수정 (ch5/5.4.3, ch6/6.2.3) | ✅ 완료 (arm64 실기 검증 + x86_64·공저자 환경 정상 회신) |
| [Gateway API](gateway-api.md) | nginx ingress → NGINX Gateway Fabric v2.6.1 (ch3/3.3.3) | ✅ 완료 |
| [kubespray](kubespray.md) | 1판→2판 변경 원인 분석 + 4개 파일 수정 (app/C.kubespray) | ✅ 완료 |
| [기타 변경](misc.md) | 저장소 구조 정리 / 파일 교정 | ✅ 완료 |
