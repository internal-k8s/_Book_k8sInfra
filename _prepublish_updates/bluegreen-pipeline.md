...
>   "파이프라인 실기 검증" 절 참고
> - ✅ **편집자 재실습 정상**, **x86_64 정상** — 회신으로 확인. 최초 보고 건 종결
>
...
[harbor.md](harbor.md)의 "ch5/5.4.3 연계 검증" 절 참고. 편집자 환경 고유 문제였고,
2026-08-08 편집자 재실습 정상 회신으로 종결되었다.
...
> **2026-08-08 종결**: 편집자 재실습이 정상으로 회신되어 Console Output 캡처로
> 실패 지점을 확정하는 일은 불필요해졌다. 아래 분석은 이 코드의 성질을 기록해 둔 것이며,
> 개정판에서 재검토할 때 근거로 쓴다.
...
- ✅ **편집자 재실습 정상** — 회신으로 확인. 최초 보고였던 "green 2차 배포 실패"가
  해소되었다. 원인은 편집자 환경 고유 문제였고(green 태그 미등록으로 추정) 책 절차 자체의
  결함은 아니었다. Console Output 캡처로 실패 지점을 확정하는 일은 더 이상 불필요
- ✅ **x86_64 환경 정상** — 회신으로 확인
- ✅ **arm64 환경 정상** — 이쪽에서 원고 9~15번 전 구간 실기 검증 ("파이프라인 실기 검증" 절)

### 남겨두는 참고 사항

- 저장소 rename 시점·주체는 GitHub API로 조회되지 않는다(k8s-edu 조직 Audit log 필요).
  수정 자체엔 불필요하다. "누가 잘못 커밋했나"는 이미 규명됨 — 아무도 잘못 넣은 게 아니라
  1판→2판 URL 정리에서 blue-green 한 건이 누락된 것이다. "경위" 절 참고
- 83번 줄의 무한 대기 문제는 개정판 몫으로 남긴다. 판단 근거와 검증해 둔 대안은
  "수정하지 않기로 한 건" 절에 있다
