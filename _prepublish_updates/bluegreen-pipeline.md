...
> - ⏳ **원고(docx) 2곳 미반영** — 원고는 **핑크 윈도 랩탑**에서 메인으로 관리하므로
>   그쪽에서 이 문서를 읽고 작업할 것. 정확한 문자열은 "변경 파일 → 원고" 절 참고
>
...
원고는 `컨테이너 - 5.지속적인 통합과 배포 자동화, 젠킨스_19July2026.docx`.
...
| 참조 | 상태 |
|---|---|
| `k8s-edu/Bkv2_sub_blue-green-pipeline` | ⚠️ **301 → `Bkv2_sub_blue-green`** (유일한 문제, 원고 2곳) |
...
| 파일 | 줄 | 변경 | 상태 |
|---|---|---|---|
| `Jenkinsfile` | 32 | `Bkv2_sub_blue-green-pipeline.git` → `Bkv2_sub_blue-green.git` | ✅ 완료 |
| `Jenkinsfile` | 83 | (변경 없음 — 원래 코드 유지) | — |
...
### 원고 — `컨테이너 - 5...젠킨스_19July2026.docx`

**⏳ 2곳 미반영. 핑크 윈도 랩탑에서 작업할 것.** 두 곳 다 5.4.3 8번 안에 있고,
`-pipeline` 8글자만 삭제하면 된다. `.git` 접미사는 유지한다.
...
| # | 위치 | 문단 위치 | Word 검색어 | 상태 |
|---|---|---|---|---|
| 1 | `<코드> Jenkinsfile` 리스팅 **032**번 줄 | 1737 / 2661 (65%) | `blue-green-pipeline` 1번째 히트 | ⏳ |
| 2 | 코드 설명 **"29~34번째 줄:"** 문단 | 1812 / 2661 (68%) | `blue-green-pipeline` 2번째 히트 | ⏳ |

`blue-green-pipeline`은 ch5 전체에서 정확히 이 2건만 나온다. 다른 절에는 없다.

반영 후 032번 줄이 저장소 최종본(`d3f8457`)과 일치해야 한다:
...
**저장소는 반영 완료. 원고 2곳만 남았다.**

원고 8번이 "실제 Jenkins 코드를 살펴보겠습니다"라며 저장소 파일을 그대로 싣는 구조이므로,
원고를 반영하기 전까지는 인쇄될 코드와 실제 코드가 032번 줄에서 다른 상태다. 반드시 맞춰야 한다.

1. ⏳ **핑크 윈도 랩탑에서 docx 2곳 반영** (서식 유지를 위해 저자가 직접 편집)
2. ⏳ 반영 후 032번 줄이 저장소 `d3f8457`과 일치하는지 대조
3. ⏳ 파이프라인 재실행으로 1차(blue)/2차(green) 통과 확인 — 아직 미수행 (아래 "미해결")

편집자 실습 종료를 기다릴 필요는 없다고 판단해 저장소는 먼저 반영했다. 32번 줄 변경이
