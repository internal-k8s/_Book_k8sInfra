...
| `ch4/4.4.2/2.harbor/2-1.get_harbor.sh` | **arm64 네임스페이스 sed 재주입 (하단 "arm64 네임스페이스 치환 재주입" 섹션)** | ✅ 적용 + 클린 환경 검증 완료 (2026-07-12) |
| `ch4/4.4.2/_image-builder/build.sh` | arm64 이미지 빌드 스크립트 (저자 전용, 리포 미포함 — 원저자 로컬에만 존재) | ✅ |
| `ch4/4.4.2/uninstall_harbor.sh` | TAG v2.10.0 → v2.15.0 + arm64 분기 추가 (위치: `4.4.2/` 직하, `2.harbor/` 아님) | ✅ |
