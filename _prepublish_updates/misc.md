...
- 부득이하게 버전을 바꿔야 할 때는 "최신 중 가장 성숙한 버전"이 아니라 **원래 목표에서 가장 적게 벗어나는
  선택**을 기준으로 고름
- **예외: Helm CLI 자체는 최신화 대상에 포함** — 쿠버네티스에 직접 연결되는 도구라 ch5/ch6 안의
  Jenkins/Prometheus/Grafana 앱 자체와는 다르게 취급
- ch5/ch6은 `edu` 커스텀 Helm 차트 저장소에 버전이 이미 고정돼 있어(별도 `--version` 없이 그 저장소의
  고정 버전이 설치됨) 원래도 최신화 대상이 아니었음
...
- 원래 목표였던 24.0.6은 Ubuntu 24.04(noble) 저장소에 아예 빌드된 적이 없어 설치 불가능
  ...
  상태라 이 하드 블로커는 피할 수 없음
- "가장 적게 벗어나는 선택" 기준으로 noble에서 설치 가능한 가장 이른 버전(26.0.0대)으로 결정 진행 중
