...
## ch6/README.md 목차를 원고 기준으로 재작성 (2026-07-18)

### 문제

`ch6/README.md`의 목차가 원고(`컨테이너 - 6...._2025Jun15.docx`)와 여러 곳에서 어긋나 있었음:

- 6.3 절 제목 및 6.3.3: README "PromQL 데이터 타입" → 원고엔 이 제목 자체가 없음, 실제는 "인스턴트 벡터와 레인지 벡터"
- 6.4: README는 4개 항목(헬름설치/데이터소스/노드메트릭/파드메트릭)뿐이었으나 원고는 6개 항목이고 파드↔노드 순서도 반대
- 6.5: README "좀 더 견고한 모니터링 환경 만들기"(로키/템포/얼럿매니저/대시보드공유×2) — 원고엔 이 절 제목 자체가 없고, 실제 6.5는 "**그라파나 활용성 높이기**"(대시보드 백업·복구/대시보드 가져오기/얼럿매니저)뿐. 로키·템포는 원고상 6.6 소속인데 README에 6.5.1/6.5.2로 중복 기재돼 있었음
- 6.6: README "쿠버네티스에 배포된 애플리케이션을 모니터링하기" → 원고는 "**모니터링 범위 확장하기**" (하위 로키/템포/파이로스코프 순서는 일치)

### 조치

`ch6/README.md`를 원고(2025Jun15본) 헤딩 구조 기준으로 전면 재작성. 원고보다 더 최신 개정본이 나오면 재대조 필요.

### docx 영향

없음 — 원고가 기준(source of truth)이고 README만 따라간 것.
