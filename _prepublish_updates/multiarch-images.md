# 멀티 플랫폼(arm64) 이미지 전수 감사 — 2건 수정 ✅ 종결
...
> - ✅ **x86_64 정상 확인** (회신), **공저자 환경 정상 확인**(캐시 정리 후 회신)
> - **남은 확인 사항 없음.** 아래 "확인 결과" 절에 경위를 남긴다
...
### 1. 노드 이미지 캐시 삭제 ✅ 처리됨 (2026-08-08, 공저자 회신)

두 건 다 **같은 태그로 덮어썼다.** 파드 스펙에 `imagePullPolicy`가 없고 태그가 `latest`가
아니면 기본값이 `IfNotPresent`이므로, 이미 옛 이미지를 받아둔 노드는 캐시를 계속 쓴다.
문성주 님 환경에서 캐시 정리 후 **정상 동작 확인 회신**을 받았다.

앞으로 같은 태그를 덮어쓸 때 재발할 수 있으므로 명령을 남겨 둔다.
...
수정했는데도 그대로 실패한다면 십중팔구 이것이다.
임시로 `line/kubectl-kustomize:1.30.1-5.4.1`로 교체해 두었다면 원복해도 된다.
...
### 4. x86_64 환경 회귀 확인 ✅ 정상 (2026-08-08, 회신으로 확인)

두 이미지의 amd64 변형은 바이너리 아키텍처와 버전을 이쪽에서 확인했고,
**x86_64 실습 동작은 별도 회신으로 정상 확인**되었다.

`nginx-status`는 베이스가 `nginx:1.18.0` -> `1.26.0`으로 올라갔으므로,
ch6 캡처를 새로 찍는다면 nginx 버전 표기가 보이는 화면이 있는지만 확인하면 된다
(현재까지 문제 보고 없음).
